...
// UX Input Arguments
pColor = #003CFF //color.blue
ycColor = #FFDB00 //color.yellow

rMainLevelColor = #FF1800 //color.red
rHalfLevelColor = color.new(#FF1800, 70) //color.red

sMainLevelColor = #00AE47 //color.green
sHalfLevelColor = color.new(#00AE47, 70) //color.green
...
fill(pivotPlot, closePlot, YC > P ? color.new(color.green, 80) : color.new(color.red, 80), title="Daily Trend Background (Pivot based)")
...
ema1Plot = plot(show4EmaLines ? ta.ema(close, ema1) : na, title="EMA1 Line", color=#66ff6c, linewidth=1)
ema2Plot = plot(show4EmaLines ? ta.ema(close, ema2) : na, title="EMA2 Line",color=#ff2334, linewidth=1)
fill(ema1Plot, ema2Plot, positifTrendLine > negatifTrendLine ? color.new(color.green, 90) : color.new(color.red, 90), title="Trend Background (EMA 1,2 based)")

plot(show4EmaLines ? ta.ema(close, ema3) : na, title="EMA3 Line", color=#f2c60d, linewidth=1)
plot(show4EmaLines ? ta.ema(close, ema4) : na, title="EMA4 Line", color=#fbae04, linewidth=2)
...
plot(showBollingerB ? bbBasis : na, title="BB SMA", color=#ff9800, offset=bbOffset)
bbUpperPlot = plot(showBollingerB ? bbUpper : na, title="BB Upper line", color=#e91e63, linewidth=2, offset=bbOffset)
bbLowerPlot = plot(showBollingerB ? bbLower : na, title="BB Lower line", color=#4caf50, linewidth=2, offset=bbOffset)
fill(bbUpperPlot, bbLowerPlot, color=color.new(#2196f3, 90), title="BB Background")
...
vwapLinesStyle = plot.style_cross
...
plot(showVwapLines ? vwapValue1 : na, color=color.new(#b2b5be, 10), style=vwapLinesStyle)
...
plot(showVwapLines ? vwapValue2 : na, color=color.new(color.white, 10), style=vwapLinesStyle,linewidth=1)
...
plot(showVwapLines ? vwapValue3 : na, color=color.new(#ffe0b2, 10), style=vwapLinesStyle, linewidth=1)
...
plot(showVwapLines ? vwapValue4 : na, color=color.new(#cfb9ff, 10), style=vwapLinesStyle, linewidth=1)
...
U5 = plot(showBands ? myvwap + std * dev5 : na, title="Upper Band 5", color=color.new(color.red, 75), offset=vwapOffset)
U4 = plot(showBands ? myvwap + std * dev4 : na, title="Upper Band 4", color=color.new(color.red, 75), offset=vwapOffset)
U3 = plot(showBands ? myvwap + std * dev3 : na, title="Upper Band 3", color=color.new(color.red, 75), offset=vwapOffset)
U2 = plot(showBands ? myvwap + std * dev2 : na, title="Upper Band 2", color=color.new(color.red, 75), offset=vwapOffset)
U1 = plot(showBands ? myvwap + std * dev1 : na, title="Upper Band 1", color=color.new(color.red, 75), offset=vwapOffset)

plot(showBands ? myvwap : na, title="VWAP", offset=vwapOffset, color=color.white)

D1 = plot(showBands ? myvwap - std * dev1 : na, title="Lower Band 1", color=color.new(color.green, 75), offset=vwapOffset)
D2 = plot(showBands ? myvwap - std * dev2 : na, title="Lower Band 2", color=color.new(color.green, 75), offset=vwapOffset)
D3 = plot(showBands ? myvwap - std * dev3 : na, title="Lower Band 3", color=color.new(color.green, 75), offset=vwapOffset)
D4 = plot(showBands ? myvwap - std * dev4 : na, title="Lower Band 4", color=color.new(color.green, 75), offset=vwapOffset)
D5 = plot(showBands ? myvwap - std * dev5 : na, title="Lower Band 5", color=color.new(color.green, 75), offset=vwapOffset)
